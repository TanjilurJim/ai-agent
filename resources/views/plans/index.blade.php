                    ...
                    <div class="usage-item">
                        ...
                        <div class="usage-progress">
                            <div class="usage-progress-bar"
                                style="width: {{ ($personalityCount / $personalityLimit) * 100 }}%"></div>
                        </div>
                    </div>
                    <div class="usage-item">
                        ...
                        <div class="usage-progress">
                            <div class="usage-progress-bar" style="width: {{ ($todayUsage / $dailyPromptLimit) * 100 }}%">
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="plan-body">
                            <ul class="plan-features">
                                <li>
                                    ...
                                    <span><strong>{{ $plan->widget_limit }}</strong>
                                        {{ $plan->widget_limit == 1 ? 'Widget' : 'Widgets' }}</span>
                                </li>
                                <li>
                                    <i class="fas fa-check-circle"></i>
                                    <span><strong>{{ $plan->personality_limit }}</strong>
                                        {{ $plan->personality_limit == 1 ? 'Personality' : 'Personalities' }}</span>
                                </li>
                                <li>
                                    <i class="fas fa-check-circle"></i>
                            ...
                            @else
                                <form method="POST" action="{{ route('plans.request') }}"
                                    class="d-inline plan-request-form" data-plan-name="{{ ucfirst($plan->name) }}">
                                    @csrf
                                    <input type="hidden" name="plan_id" value="{{ $plan->id }}">
                                    <input type="hidden" name="contact_number" value="">
                                    <button type="button" class="plan-button primary plan-request-btn">
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            document.querySelectorAll('.plan-request-btn').forEach(function(btn) {
                btn.addEventListener('click', function() {
                    const form = this.closest('.plan-request-form');
                    const planName = form.getAttribute('data-plan-name');

                    if (!confirm(`Are you sure you want to request the ${planName} plan?`)) {
                        return;
                    }

                    const contact = prompt("Please enter your contact number (WhatsApp / phone):");
                    if (!contact || !contact.trim()) {
                        alert('Contact number is required to proceed.');
                        return;
                    }

                    form.querySelector('input[name="contact_number"]').value = contact.trim();
                    form.submit();
                });
            });
        });
