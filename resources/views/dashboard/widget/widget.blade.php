@section('css')
  ...
  <style>
    ...
    @media (max-width: 575.98px){
      ...
      #chat-widget .chat-box{
        width: 100% !important;
        max-width: 320px;           /* slightly smaller for iPhone SE width */
      ...
      .messages{
        max-height: 55vh;           /* use viewport height on phones */
        overflow: auto;
      }
    ...
    @media (min-width: 576px) and (max-width: 991.98px){
      ...
      .chat-box-right .chat-footer{ position: static !important; }
      .messages{ max-height: 60vh; overflow: auto; }
    }

    /* --- Desktop keeps your original look --- */
    @media (min-width: 992px){
      .chat-box-left{ width: 340px; float: left; height: 710px; }
      .chat-box-right{
        width: auto; height: 710px; margin-left: 361px;
        position: relative;
      }
      .chat-box-right .chat-footer{
    ...
    small.text-[#]{ color: inherit; } /* no-op safeguard */
  ...
  .chat-footer{
    display:flex;
    align-items:center;
    gap:.5rem;          /* space between input & button */
    padding:.75rem;     /* keep some padding */
  }

  /* Input: grow, but allow shrinking on tiny screens */
  .chat-footer input{
    flex:1 1 auto;      /* grow + shrink */
    min-width:0;        /* lets it actually shrink */
  }
